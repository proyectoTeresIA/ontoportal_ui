...
// Get config from jQuery data (set by layout in bp_config_json)
var BP_CONFIG = jQuery(document).data().bp ? jQuery(document).data().bp.config : {};
...
function get_link_for_cls_ajax(cls_id, ont_acronym, isOntoLex) {
  ...
  var data_ontolex = isOntoLex ? " data-ontolex='true' " : '';
  return (
    "<a class='cls4ajax'" +
    data_cls +
    data_ont +
    data_ontolex +
    "href='" +
    bp_cls_link(cls_id, ont_acronym, isOntoLex) +
    "'>" +
    cls_id +
    '</a>'
  );
...
var ajax_process_cls_interval = null,
...
var ajax_process_ont = function () {
  ...
  var ajax_uri = basePath + '/ajax/json_ontology/?ontology=' + encodeURIComponent(ont_acronym);
  jQuery.ajax({
    url: ajax_uri,
