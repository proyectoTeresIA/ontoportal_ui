      ...
      success: function (data) {
        ...
        if (data) {
          if (data.length > 0) {
            ...
            var header = $(
              '<thead><tr><th title="Position of the ontology in the ranking">POS.</th>' +
                '<th title="Ontology acronym">' +
                ontologyHeader +
                '</th>' +
            ...
            for (var i = 0; i < data.length; i++) {
              ...
              for (var j = 0; j < data[i].coverageResult.annotations.length; j++) {
                terms +=
                  ...
                  data[i].coverageResult.annotations[j].annotatedClass.links.ui +
                  '>' +
                  data[i].coverageResult.annotations[j].text +
                  '</a>, ';
              }
              ...
              var acceptanceScore =
                data[i].acceptanceResult && data[i].acceptanceResult.normalizedScore
                  ? data[i].acceptanceResult.normalizedScore * 100
                  : 0;
              var detailScore =
                data[i].detailResult && data[i].detailResult.normalizedScore
                  ? data[i].detailResult.normalizedScore * 100
                  : 0;
              var specializationScore =
                data[i].specializationResult && data[i].specializationResult.normalizedScore
                  ? data[i].specializationResult.normalizedScore * 100
                  : 0;
              ...
              row +=
                ...
                '%;" class="final-score"><span class="score-number">' +
                finalScore.toFixed(1) +
                '</span></div></td>' +
                '<td><div style="width:' +
                coverageScore.toFixed(0) +
                '%;" class="result-scores"><span class="score-number">' +
                coverageScore.toFixed(1) +
                '</span></div></td>' +
                '<td><div style="width:' +
                acceptanceScore.toFixed(0) +
                '%;" class="result-scores"><span class="score-number">' +
                acceptanceScore.toFixed(1) +
                '</span></div></td>' +
                '<td><div style="width:' +
                detailScore.toFixed(0) +
                '%;" class="result-scores"><span class="score-number">' +
                detailScore.toFixed(1) +
                '</span></div></td>' +
                '<td><div style="width:' +
                specializationScore.toFixed(0) +
                '%;" class="result-scores"><span class="score-number">' +
                specializationScore.toFixed(1) +
                '</span></div></td>' +
                '<td>' +
                data[i].coverageResult.annotations.length +
                '</td>' +
                '<td>' +
                '<div style="text-align:center"><input style="vertical-align:middle" id="chk' +
                i +
                '" type="checkbox"/></div>' +
                '</tr>';
            ...
            // No results
            if ($('#radioOtSets').is(':checked')) $('#noResultsSets').show();
            else $('#noResults').show();
          }
        }
      },
