...
export default class extends Controller {
  ...
  connect() {
    setTimeout(() => {
      ...
      // Add loading spinner next to the ID
  ...
  #showLoadingSpinner() {
    // Add a small spinner after the text
    const spinner =
      '<span class="label-ajax-spinner" style="margin-left: 8px; display: inline-block; width: 12px; height: 12px; border: 2px solid #ccc; border-top-color: #333; border-radius: 50%; animation: label-ajax-spin 0.8s linear infinite;"></span>';
    this.linkA.append(spinner);

    // Add the animation style if not already present
    if (!document.getElementById('label-ajax-spinner-style')) {
      const style = document.createElement('style');
      style.id = 'label-ajax-spinner-style';
      style.textContent = '@keyframes label-ajax-spin { to { transform: rotate(360deg); } }';
      document.head.appendChild(style);
    }
  }

  #hideLoadingSpinner() {
    this.linkA.find('.label-ajax-spinner').remove();
  }
