...
function bpMinVersion(min, actual) {
  ...
  if (actualParsed.major > minParsed.major) {
    return true;
  } else if (actualParsed.major == minParsed.major && actualParsed.minor > minParsed.minor) {
    return true;
  } else if (
    actualParsed.major == minParsed.major &&
...
if (typeof jQuery == 'undefined') {
  var jq,
    jqMigrate,
    scriptLoc = document.getElementsByTagName('script')[0].parentElement;
...
function bpQuickJumpOnLoad() {
  jQuery(document).ready(function () {
    ...
    if (
      jQuery('link[href$="' + BP_SEARCH_SERVER + '/javascripts/JqueryPlugins/autocomplete/jquery.autocomplete.css"]')
    ) {
    ...
    var i18n = window.BP_I18N || {};
    var jumpLabel = i18n.jump_to_label || 'Jump To:';
    var goToBtn = i18n.go_to_button || 'Go to ' + BP_SITE;
    jQuery('#bp_quick_jump').append(
      jumpLabel +
        ' <input type="textbox" id="' +
        jumpTo_searchBoxID +
        '"> <input type="button" value="' +
        goToBtn +
        '" onclick="jumpTo_jump_clicked();">',
    );
    ...
    getScript(BP_SEARCH_SERVER + '/javascripts/JqueryPlugins/autocomplete/crossdomain_autocomplete.js').then(
      function () {
...
function jumpTo_jumpToValue(li) {
  if (jQuery('#jump_to_concept_id') == null && jQuery('#jump_to_ontology_id') == null) {
    var i18n = window.BP_I18N || {};
    var confirmMsg =
      i18n.not_found_confirm ||
      'Class could not be found or is not browsable in ' +
        BP_SITE +
        '.\n\nPress OK to go to the ' +
        BP_SITE +
        ' Search page or Cancel to try again';
    var search = confirm(confirmMsg);
    if (search) {
  ...
  if (jQuery('#jump_to_concept_id') != null && jQuery('#jump_to_ontology_id') != null) {
    ...
    document.location =
      BP_SEARCH_SERVER + '/ontologies/' + ontology_id + '/?p=classes&conceptid=' + encodeURIComponent(sValue);
    return;
  }
}
...
function jumpTo_formatItem(row) {
  ...
  if (row[0].match(regex) == null) {
    ...
    if (synonym !== '') {
      ...
      var i18n = window.BP_I18N || {};
      var synPref = i18n.synonyms_prefix || 'synonyms:';
      row[0] = row[0] + ' (' + synPref + ' ' + matchSynonym.join(', ') + ')';
    }
  }
  ...
  if (row[0].indexOf('[obsolete]') != -1) {
    ...
    var i18n = window.BP_I18N || {};
    var obsoleteTitle = i18n.obsolete_class_title || 'obsolete class';
    obsolete_prefix = "<span class='obsolete_class' title='" + obsoleteTitle + "'>";
...
function definitionDiv(ont, concept) {
  ...
  definitionAjax.attr(
    'href',
    BP_SEARCH_SERVER + '/ajax/json_class?callback=?&ontologyid=' + ont + '&conceptid=' + encodeURIComponent(concept),
  );
  var definitionDiv = jQuery('<div>');
  definitionDiv.addClass('result_definition');
  var i18n = window.BP_I18N || {};
  definitionDiv.text(i18n.retrieving_definitions || 'retrieving definitions...');
...
function jumpTo_setup_functions() {
  ...
  jQuery(jumpTo_searchBoxSelector).bioportal_autocomplete(BP_SEARCH_SERVER + '/search/json_search/' + BP_ontology_id, {
    ...
    footer: (function () {
      var i18n = window.BP_I18N || {};
      var provided = i18n.results_provided_by || 'Results provided by';
      return (
        '<div style="color: grey; font-size: 8pt; font-family: Verdana; padding: .8em .5em .3em;">' +
        provided +
...
function truncateText(text, max_length) {
  ...
  if (content_length <= max_length) return text; // bail early if not overlong
